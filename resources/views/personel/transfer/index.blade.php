<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-lg sm:text-xl text-slate-800 dark:text-slate-200 leading-tight">
            {{ __('Transfer Saldo') }}
        </h2>
    </x-slot>

    <div class="py-4 sm:py-12 bg-slate-50 dark:bg-slate-800/50 min-h-screen px-2 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 space-y-4 sm:space-y-8">

            <!-- Saldo Card with Gradient -->
            <div
                class="relative overflow-hidden rounded-xl sm:rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-700 p-5 sm:p-8 shadow-xl text-slate-900 dark:text-white">
                <div
                    ...
                    <div
                        class="p-2 sm:p-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5/10 rounded-full border border-slate-400 dark:border-white/20 hidden sm:block">
                ...
                <!-- Decorative Circles -->
                <div
                    class="absolute top-0 right-0 -mt-10 -mr-10 w-32 h-32 sm:w-40 sm:h-40 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5/10 rounded-full blur-3xl">
                </div>
                <div
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-8">
                ...
                <div class="lg:col-span-1">
                    <div
                        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl shadow-sm border border-slate-200 dark:border-white/5 overflow-hidden sticky top-4 sm:top-8">
                        <div class="p-4 sm:p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-800/50">
                            <h3 class="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-200 flex items-center">
                            ...
                            <p class="text-xs sm:text-xs text-slate-600 dark:text-slate-400 mt-1">Kirim saldo ke rekan satu Satker.</p>
                        </div>
                        ...
                        <div class="p-4 sm:p-6" x-data="{
                            ...
                            <form method="post" action="{{ route('personel.transfer.store') }}"
                                ...
                                <div class="p-1 bg-slate-50 dark:bg-slate-800 rounded-xl flex mb-4">
                                    <button type="button" @click="tipeTujuan = 'personel'"
                                        class="flex-1 px-4 py-2 text-xs font-bold rounded-lg transition-all"
                                        :class="tipeTujuan === 'personel' ? 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 text-indigo-600 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300'">
                                        Ke Rekan
                                    </button>
                                    <button type="button" @click="tipeTujuan = 'kendaraan'"
                                        class="flex-1 px-4 py-2 text-xs font-bold rounded-lg transition-all"
                                        :class="tipeTujuan === 'kendaraan' ? 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 text-indigo-600 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300'">
                                        Ke Kendaraan
                                    </button>
                                </div>
                                ...
                                <div x-show="tipeTujuan === 'personel'">
                                    <x-input-label for="receiver_id" :value="__('Penerima')"
                                        class="text-xs sm:text-xs text-slate-700 dark:text-slate-300" />
                                    ...
                                    <div class="relative mt-1" @click.outside="receiverOpen = false">
                                        <div @click="receiverOpen = !receiverOpen; $nextTick(() => { if(receiverOpen) $refs.receiverInput.focus() })"
                                            class="w-full pl-9 sm:pl-10 pr-4 py-2 sm:py-2.5 rounded-lg sm:rounded-xl border border-slate-400 dark:border-white/20 text-xs sm:text-sm shadow-sm transition-all duration-200 cursor-pointer flex items-center justify-between bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5"
                                            :class="receiverOpen ? 'ring-2 ring-indigo-500 border-indigo-500' : ''">
                                            <span x-text="receiverLabel || '-- Pilih Rekan --'"
                                                :class="receiverLabel ? 'text-slate-800 dark:text-slate-200' : 'text-slate-600 dark:text-slate-400'"
                                                class="truncate"></span>
                                            <svg class="w-4 h-4 text-slate-600 dark:text-slate-400 transition-transform shrink-0"
                                        ...
                                        <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none text-slate-600 dark:text-slate-400"
                                            x-show="!receiverOpen">
                                        ...
                                        <div x-show="receiverOpen" x-transition.opacity.duration.150ms
                                            class="absolute z-50 mt-1 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 border border-slate-300 dark:border-white/10 rounded-xl shadow-lg overflow-hidden"
                                            style="display:none;">
                                            <div class="bg-white dark:bg-slate-900 dark:text-white p-2 border-b border-slate-200 dark:border-white/5">
                                                <div class="relative">
                                                    ...
                                                    <input x-ref="receiverInput" x-model="receiverSearch" type="text"
                                                        ...
                                                        class="bg-white dark:bg-slate-900 dark:text-white w-full pl-9 pr-3 py-2 text-sm border border-slate-300 dark:border-white/10 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                                </div>
                                            </div>
                                            <div class="max-h-48 overflow-y-auto">
                                                <template x-for="r in filteredReceivers" :key="r.id">
                                                    <div @click="selectReceiver(r)"
                                                        class="px-4 py-2.5 text-xs sm:text-xs text-slate-700 dark:text-slate-300 hover:bg-indigo-50 cursor-pointer flex items-center justify-between transition-colors"
                                    ...
                                    <p class="mt-1.5 text-[10px] sm:text-xs text-slate-600 dark:text-slate-400 flex items-center">
                                    ...
                                    <div class="relative mt-1" @click.outside="kendaraanOpen = false">
                                        <div @click="kendaraanOpen = !kendaraanOpen; $nextTick(() => { if(kendaraanOpen) $refs.kendaraanInput.focus() })"
                                            ...
                                            <span x-text="kendaraanLabel || '-- Pilih Kendaraan --'"
                                                :class="kendaraanLabel ? 'text-slate-800 dark:text-slate-200' : 'text-slate-600 dark:text-slate-400'"
                                                class="truncate"></span>
                                            <svg class="w-4 h-4 text-slate-600 dark:text-slate-400 transition-transform shrink-0"
                                        ...
                                        <div x-show="kendaraanOpen" x-transition.opacity.duration.150ms
                                            ...
                                            <div class="max-h-48 overflow-y-auto">
                                                ...
                                                <div x-show="filteredKendaraans.length === 0"
                                                    class="px-4 py-3 text-xs text-slate-600 dark:text-slate-400 text-center">Tidak ditemukan atau BBM tidak cocok
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div>
                                    <x-input-label for="jumlah" :value="__('Jumlah Transfer')"
                                        class="text-xs sm:text-xs text-slate-700 dark:text-slate-300 font-medium" />
                                    <div class="relative mt-1 sm:mt-2">
                                        <input id="jumlah" name="jumlah" type="number" x-model.number="jumlah"
                                            class="w-full pl-4 pr-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl border-slate-300 dark:border-white/10 bg-slate-50 focus:bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-slate-800 dark:text-slate-200 font-bold text-base sm:text-lg shadow-sm transition-all duration-200"
                                ...
                                <div>
                                    ...
                                    <div class="relative mt-1 sm:mt-2">
                                        <div
                                            class="absolute inset-y-0 left-0 flex items-center pl-3 sm:pl-4 pointer-events-none text-slate-600 dark:text-slate-400">
                                            <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor"
                                                viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        ...
                                        <input id="pin" name="pin" type="password"
                                            class="w-full pl-10 sm:pl-12 pr-4 py-2.5 sm:py-3 rounded-lg sm:rounded-xl border-slate-300 dark:border-white/10 bg-slate-50 focus:bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 text-slate-800 dark:text-slate-200 font-bold text-base sm:text-lg shadow-sm transition-all duration-200 tracking-[0.5em]"
                ...
                <div class="lg:col-span-2">
                    <div
                        class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-xl sm:rounded-2xl shadow-sm border border-slate-200 dark:border-white/5 overflow-hidden h-full">
                        <div
                            class="p-4 sm:p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-800/50 flex justify-between items-center">
                            <h3 class="text-base sm:text-lg font-bold text-slate-800 dark:text-slate-200 flex items-center">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 mr-2 text-indigo-600" fill="none"
                                    stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr
                                        class="bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 text-[10px] sm:text-xs font-bold uppercase tracking-wider border-b border-slate-300 dark:border-white/10">
                                ...
                                <tbody class="divide-y divide-slate-200 dark:divide-white/5">
                                    @forelse($riwayat as $item)
                                        <tr class="hover:bg-slate-50 dark:bg-slate-800/80 transition-colors duration-150">
                                            <td class="p-3 sm:p-4 whitespace-nowrap">
                                                <div class="flex flex-col">
                                                    <span
                                                        class="text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300">{{ $item->created_at->timezone('Asia/Makassar')->format('d M Y') }}</span>
                                                    <span
                                                        class="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400">{{ $item->created_at->timezone('Asia/Makassar')->format('H:i') }}
                                                        WITA</span>
                                                </div>
                                            </td>
                                            <td class="p-3 sm:p-4 whitespace-nowrap">
                                            ...
                                            <td class="p-3 sm:p-4 whitespace-nowrap">
                                                @if($item->sender_id == $personel->id)
                                                    ...
                                                    <div class="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400">Penerima</div>
                                                @else
                                                    <div class="text-xs sm:text-sm font-medium text-slate-900 dark:text-white">
                                                        {{ $item->sender->nama ?? 'Tidak Diketahui' }}
                                                    </div>
                                                    <div class="text-[10px] sm:text-xs text-slate-600 dark:text-slate-400">Pengirim</div>
                                                @endif
                                            </td>
                                            ...
                                            <td
                                                class="p-3 sm:p-4 text-xs sm:text-xs text-slate-600 dark:text-slate-400 truncate max-w-xs hidden sm:table-cell">
                                                {{ $item->keterangan ?? '-' }}
                                            </td>
                                        </tr>
                                    @empty
                                        <tr>
                                            <td colspan="5" class="p-6 sm:p-8 text-center">
                                                <div class="flex flex-col items-center justify-center text-slate-600 dark:text-slate-400">
                        ...
                        @if($riwayat->hasPages())
                            <div class="p-3 sm:p-4 border-t border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-800/50">
                                {{ $riwayat->links() }}
                            </div>
                        @endif
                    </div>
