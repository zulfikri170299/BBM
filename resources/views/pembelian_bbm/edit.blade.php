<x-app-layout>
    <div class="p-6 lg:p-8 space-y-8 max-w-3xl mx-auto">
        <div class="flex items-center gap-4 mb-6">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-900 leading-tight">Edit Data Pembelian BBM</h1>
                <p class="text-sm text-slate-500 mt-1">Ubah data riwayat pembelian BBM Pertamax & Pertamina Dex.</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <form action="{{ route('pembelian-bbm.update', $pembelianBbm->id) }}" method="POST" class="p-4 sm:p-6 space-y-5" autocomplete="off">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', \Carbon\Carbon::parse($pembelianBbm->tanggal)->format('Y-m-d')) }}" class="flatpickr w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" required>
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="jumlah" value="{{ old('jumlah', $pembelianBbm->jumlah) }}" step="0.01" min="0.01" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" placeholder="Masukkan jumlah liter..." required>
                </div>
                
                <div class="pt-4 mt-6 border-t border-slate-100 flex gap-3">
                    <a href="{{ route('pembelian-bbm.index') }}" class="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-all active:scale-95 flex items-center justify-center">
                        Batal
                    </a>
