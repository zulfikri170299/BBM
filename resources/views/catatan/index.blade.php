<x-app-layout>
    <div class="py-8 px-2 sm:px-6 lg:px-8 bg-slate-800/50/30 min-h-screen" x-data="{ 
        showAddModal: false,
        showEditModal: false,
        ...
        colors: [
            ...
            { name: 'Slate', class: 'bg-slate-800/50 border-white/10 text-slate-300', value: 'slate' }
        ],
        editNote(note) {
        ...
        <div class="max-w-7xl mx-auto mb-8 px-2 sm:px-6 lg:px-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
                <div>
                    <h1 class="text-2xl font-bold text-white tracking-tight">Catatan & Memo</h1>
                    <p class="mt-1 text-xs text-slate-400 font-medium italic">Simpan ide, pengingat, atau catatan penting Anda di sini.</p>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
            @if($catatans->isEmpty())
                <div class="flex flex-col items-center justify-center py-20 bg-slate-900 border border-white/5 rounded-3xl border-2 border-dashed border-white/10">
                    ...
                    <h3 class="text-lg font-bold text-white">Belum Ada Catatan</h3>
                    <p class="text-slate-400 text-sm mt-1">Mulai buat catatan pertama Anda hari ini.</p>
                </div>
            @else
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    @foreach($catatans as $catatan)
                        <div class="group relative flex flex-col p-6 rounded-2xl border-2 transition-all duration-300 hover:shadow-xl hover:-translate-y-1 {{ 
                            ...
                            $catatan->warna == 'slate' ? 'bg-slate-800/50 border-white/5 text-white' : 'bg-indigo-50 border-indigo-100 text-indigo-900')))) 
                        }}">
                            <!-- Note Actions -->
                            <div class="absolute top-4 right-4 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button @click="editNote({
                                    ...
                                    isi: @js($catatan->isi),
                                    warna: '{{ $catatan->warna }}'
                                })" class="p-1.5 rounded-lg hover:bg-slate-900 border border-white/5/50 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path>
                                    </svg>
                                </button>
                                <button @click="confirmDelete('{{ $catatan->id }}')" class="p-1.5 rounded-lg hover:bg-slate-900 border border-white/5/50 text-rose-600 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div x-show="showAddModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-slate-900 border border-white/5 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form action="{{ route('catatan.store') }}" method="POST">
                        ...
                        <div class="bg-slate-900 border border-white/5 px-8 pt-8 pb-6">
                            ...
                            <div class="space-y-5">
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Judul</label>
                                    <input type="text" name="judul" required placeholder="Judul catatan..." class="w-full px-4 py-3 bg-slate-800/50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Isi Catatan</label>
                                    <textarea name="isi" required rows="6" placeholder="Tulis sesuatu di sini..." class="w-full px-4 py-3 bg-slate-800/50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium"></textarea>
                                </div>
                                <div>
                        ...
                        <div class="bg-slate-800/50 px-8 py-6 flex flex-col sm:flex-row-reverse gap-3">
                            ...
                            <button type="button" @click="showAddModal = false" class="w-full sm:w-auto inline-flex justify-center rounded-xl px-8 py-3 text-sm font-bold text-slate-300 hover:bg-gray-200 transition-all active:scale-95">
                                Batal
                            </button>
                        </div>
                    </form>
                    ...
                    <form :action="`/catatan/${currentNote.id}`" method="POST">
                        ...
                        <div class="bg-slate-900 border border-white/5 px-8 pt-8 pb-6">
                            <div class="mb-6 flex items-center justify-between">
                                ...
                                <button type="button" @click="showEditModal = false" class="text-gray-400 hover:text-slate-400">
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                </button>
                            </div>
                            ...
                            <div class="space-y-5">
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Judul</label>
                                    <input type="text" name="judul" required x-model="currentNote.judul" class="w-full px-4 py-3 bg-slate-800/50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Isi Catatan</label>
                                    <textarea name="isi" required rows="6" x-model="currentNote.isi" class="w-full px-4 py-3 bg-slate-800/50 border-0 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all font-medium"></textarea>
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Warna</label>
                        ...
                        <div class="bg-slate-800/50 px-8 py-6 flex flex-col sm:flex-row-reverse gap-3">
                            ...
                            <button type="button" @click="showEditModal = false" class="w-full sm:w-auto inline-flex justify-center rounded-xl px-8 py-3 text-sm font-bold text-slate-300 hover:bg-gray-200 transition-all active:scale-95">
                                Batal
                            </button>
                        </div>
                    </form>
        ...
        <div x-cloak x-show="showDeleteModal" class="fixed inset-0 z-[9999] overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-slate-900 border border-white/5 rounded-3xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
                    <div class="bg-slate-900 border border-white/5 px-8 pt-8 pb-6">
                        <div class="flex items-center gap-4 mb-6">
                            ...
                            <div>
                                <h3 class="text-xl font-bold text-white">Hapus Catatan?</h3>
                                <p class="text-xs text-slate-400 font-medium">Tindakan ini tidak dapat dibatalkan.</p>
                            </div>
                        </div>
                        <p class="text-slate-400 text-sm leading-relaxed">Apakah Anda yakin ingin menghapus catatan ini secara permanen?</p>
                    </div>
                    <div class="bg-slate-800/50 px-8 py-6 flex flex-col sm:flex-row-reverse gap-3">
                        ...
                        <button type="button" @click="showDeleteModal = false" class="w-full sm:w-auto inline-flex justify-center rounded-xl px-8 py-3 text-sm font-bold text-slate-300 hover:bg-gray-200 transition-all active:scale-95">
                            Batal
                        </button>
                    </div>
                </div>
