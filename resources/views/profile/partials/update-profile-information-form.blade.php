<section>
    <header class="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
        <div class="p-2 sm:p-3 bg-indigo-100 text-indigo-600 rounded-2xl shadow-sm">
        ...
        <div>
            <h2 class="text-lg sm:text-xl font-bold text-slate-900 tracking-tight"> Informasi Profil </h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-0.5"> Perbarui informasi dasar akun dan alamat email Anda.
            </p>
        </div>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="space-y-4 sm:space-y-6 max-w-xl">
        @csrf
        @method('patch')

        <div>
            <label for="name" class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1 sm:mb-2">Nama
                Lengkap</label>
            ...
            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && !$user->hasVerifiedEmail())
                <div class="mt-3 p-3 bg-amber-50 rounded-xl border border-amber-100">
                    ...
                    <button form="send-verification"
                        class="mt-2 text-xs font-bold text-indigo-600 hover:text-indigo-800 underline">
        ...
        <div class="flex items-center gap-4 pt-2">
            <button type="submit"
                class="inline-flex items-center px-4 sm:px-6 py-2 sm:py-2.5 bg-indigo-600 text-white font-bold text-xs sm:text-sm rounded-xl shadow-lg shadow-indigo-500/30 hover:shadow-xl hover:shadow-indigo-500/40 transition-all transform hover:-translate-y-0.5">
