    ...
    <header class="flex items-center gap-3 sm:gap-4 mb-6 sm:mb-8">
        ...
        <div>
            @if(auth()->user()->topup_password)
                ...
                <p class="text-xs sm:text-sm text-slate-500 mt-0.5"> Masukkan password login Anda untuk keamanan, lalu buat
                    password baru. </p>
            @else
    ...
    <form method="post" action="{{ route('profile.topup-password.update') }}" class="space-y-4 sm:space-y-6 max-w-xl">
        ...
        <div>
            <label for="current_password_topup"
                class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1 sm:mb-2">Password Login
                (Verifikasi)</label>
