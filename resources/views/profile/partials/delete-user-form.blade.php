<section class="space-y-6">
    <header class="flex items-center gap-4 mb-4">
        ...
        <div>
            <h2 class="text-xl font-bold text-white tracking-tight"> Hapus Akun </h2>
            <p class="text-sm text-slate-400 mt-0.5"> Menghapus akun Anda akan menghapus semua data secara permanen. </p>
        </div>
    </header>
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-8">
            ...
            <div class="text-center">
                ...
                <h2 class="text-2xl font-bold text-white mb-2 font-display">Apakah Anda yakin?</h2>
            ...
            <div class="space-y-4 max-w-sm mx-auto">
                <div class="relative group">
                    ...
                    <input id="password" name="password" type="password" required
                        class="w-full pl-12 pr-4 py-3 bg-slate-800/50 border border-white/10 rounded-xl text-slate-200 focus:outline-none focus:ring-2 focus:ring-rose-500/20 focus:border-rose-500 transition-all placeholder-slate-400"
            ...
            <div class="mt-10 flex flex-col sm:flex-row gap-3 justify-center">
                <button type="button" x-on:click="$dispatch('close')" 
                    class="px-6 py-2.5 bg-slate-800 text-slate-400 font-bold rounded-xl hover:bg-slate-200 transition">
                    Batal
                </button>
                <button type="submit" 
