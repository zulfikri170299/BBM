<x-app-layout>
    <div class="p-2 sm:p-6 lg:p-8 space-y-4 sm:space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">Pengaturan Profil</h1>
                <p class="mt-1 text-xs sm:text-base text-slate-600 dark:text-slate-400">Kelola informasi akun, keamanan, dan preferensi
                    Anda.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div class="lg:col-span-1 space-y-4 sm:space-y-6">
                <div
                    class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-300 dark:border-white/10 shadow-sm overflow-hidden p-4 sm:p-6 text-center">
                    <div
                    ...
                    <h3 class="text-base sm:text-lg font-bold text-slate-900 dark:text-white">{{ Auth::user()->name }}</h3>
                    <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-medium break-all">{{ Auth::user()->email }}</p>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-300 dark:border-white/10 shadow-sm overflow-hidden p-2">
                    <nav class="space-y-1">
                        <a href="#profile-info"
                            class="flex items-center gap-3 px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-800/50 rounded-xl transition-colors">
            ...
            <div class="lg:col-span-2 space-y-4 sm:space-y-6">
                <!-- Profile Information -->
                <div id="profile-info"
                    class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-300 dark:border-white/10 shadow-sm overflow-hidden scroll-mt-6">
                ...
                <div id="update-photo"
                    ...
                    <div class="p-2 sm:p-6 lg:p-8">
                        <header>
                            <h2 class="text-base sm:text-lg font-medium text-slate-900 dark:text-white">
                                {{ __('Foto Profil') }}
                            </h2>
                        ...
                        <form method="post" action="{{ route('profile.photo.update') }}"
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="w-full">
                                    <input id="photo" name="photo" type="file" class="block w-full text-xs sm:text-sm text-slate-600 dark:text-slate-400
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                @if (session('status') === 'photo-updated')
                                    <p x-data="{ show: true }" x-show="show" x-transition
                                        ...
                                        class="text-xs sm:text-sm text-slate-600 dark:text-slate-400">{{ __('Saved.') }}</p>
                                @endif
                            </div>
                        </form>
                    </div>
        ...
        document.addEventListener('turbo:load', function () {
            @if(session('status') === 'password-updated')
                Swal.fire({
                    ...
                    background: '#ffffff',
                    customClass: {
                        popup: 'rounded-3xl border border-slate-200 dark:border-white/5 shadow-2xl',
                        title: 'text-xl font-bold text-slate-800 dark:text-slate-200',
                        htmlContainer: 'text-slate-600 dark:text-slate-400 font-medium text-sm',
