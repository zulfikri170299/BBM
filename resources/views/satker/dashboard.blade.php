    ...
    <div class="p-6 lg:p-8 space-y-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="mt-1 text-slate-500">Selamat datang, {{ Auth::user()->name }}. {{ Auth::user()->satker->nama_satker ?? '' }}</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
            ...
            <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-violet-500 to-purple-700 p-6 text-white shadow-xl group hover:shadow-violet-500/40 transition-all duration-300 hover:-translate-y-1">
                ...
                <div class="relative z-10">
                    ...
                    <p class="text-sm text-violet-100 mt-1">Total Transaksi BBM</p>
                </div>
            </div>
            ...
            <div class="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 p-6 text-white shadow-xl group hover:shadow-amber-500/40 transition-all duration-300 hover:-translate-y-1">
                ...
                <div class="relative z-10">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('satker.kendaraans.laporan-transfer') }}" class="text-xs font-semibold bg-white/20 hover:bg-white/30 px-3 py-1 rounded-lg transition">Laporan →</a>
                    </div>
                    <p class="text-3xl font-extrabold">{{ $totalTransfer }}</p>
                    <p class="text-sm text-amber-100 mt-1">Total Transfer</p>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div class="bg-white rounded-2xl border border-slate-200/70 p-6 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm text-slate-500">Saldo Kendaraan</p>
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($totalSaldoKendaraan, 0, ',', '.') }} <span class="text-sm font-medium text-slate-400">Liter</span></p>
                        <div class="mt-2 flex flex-wrap gap-1">
                            @foreach($saldoKendaraanPerBbm as $bbm => $total)
                                <span class="text-[9px] font-bold bg-sky-50 text-sky-700 px-1.5 py-0.5 rounded border border-sky-100 uppercase">{{ $bbm }}: {{ number_format($total, 0, ',', '.') }} L</span>
                            @endforeach
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/70 p-6 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm text-slate-500">Saldo Personel</p>
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($totalSaldoPersonel, 0, ',', '.') }} <span class="text-sm font-medium text-slate-400">Liter</span></p>
                        <div class="mt-2 flex flex-wrap gap-1">
                            @foreach($saldoPersonelPerBbm as $bbm => $total)
                                <span class="text-[9px] font-bold bg-pink-50 text-pink-700 px-1.5 py-0.5 rounded border border-pink-100 uppercase">{{ $bbm }}: {{ number_format($total, 0, ',', '.') }} L</span>
                            @endforeach
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/70 p-6 shadow-sm hover:shadow-md transition-shadow">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-sm text-slate-500">Total Liter Transfer</p>
                        <p class="text-2xl font-bold text-slate-900">{{ number_format($totalLiterTransfer, 0, ',', '.') }} <span class="text-sm font-medium text-slate-400">Liter</span></p>
                        <div class="mt-2 flex flex-wrap gap-1">
                            @foreach($literTransferPerBbm as $bbm => $total)
                                <span class="text-[9px] font-bold bg-teal-50 text-teal-700 px-1.5 py-0.5 rounded border border-teal-100 uppercase">{{ $bbm }}: {{ number_format($total, 0, ',', '.') }} L</span>
                            @endforeach
                        </div>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div class="xl:col-span-2 bg-white rounded-2xl border border-slate-200/70 shadow-sm p-6">
                <h3 class="text-lg font-bold text-slate-800 mb-4">Transfer 7 Hari Terakhir</h3>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm">
                <div class="p-6 border-b border-slate-100">
                    <h3 class="text-lg font-bold text-slate-800">Aktivitas Transfer Terbaru</h3>
                </div>
                <div class="divide-y divide-slate-100 max-h-[400px] overflow-y-auto">
                    @forelse($recentTransfers as $trx)
                    <div class="px-6 py-4 hover:bg-slate-50 transition-colors">
                        <div class="flex items-center gap-3">
                            <div class="flex-shrink-0 w-10 h-10 rounded-full bg-emerald-100 flex items-center justify-center text-emerald-600 font-bold text-sm">
                                {{ substr($trx->kendaraan->no_polisi ?? '?', 0, 2) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-semibold text-slate-800 truncate">{{ $trx->kendaraan->no_polisi ?? '-' }} → {{ $trx->personel->nama ?? '-' }}</p>
                                <p class="text-xs text-slate-400">{{ $trx->created_at->diffForHumans() }}</p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-bold text-emerald-600">{{ number_format($trx->jumlah, 0, ',', '.') }} L</p>
                            </div>
                        </div>
                    </div>
                    @empty
