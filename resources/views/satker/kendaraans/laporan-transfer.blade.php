    ...
    <div class="p-6 lg:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('satker.kendaraans.index') }}" class="inline-flex items-center px-4 py-2.5 bg-slate-100 text-slate-600 rounded-xl font-semibold text-sm hover:bg-slate-200 transition-all duration-200">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm p-5">
            <form action="{{ route('satker.kendaraans.laporan-transfer') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
                <div class="w-full md:w-auto">
                    ...
                    <input type="date" name="start_date" value="{{ request('start_date') }}" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all">
                </div>
                <div class="w-full md:w-auto">
                    <label class="flex items-center gap-1.5 text-xs font-semibold text-slate-500 mb-1.5">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                    ...
                    <input type="date" name="end_date" value="{{ request('end_date') }}" class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/10 transition-all">
                </div>
                <div class="flex gap-2">
                    <button type="submit" class="inline-flex items-center px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl font-semibold text-sm hover:from-emerald-600 hover:to-teal-700 shadow-lg shadow-emerald-500/20 transition-all duration-200 hover:-translate-y-0.5">
                    ...
                    <a href="{{ route('satker.kendaraans.laporan-transfer') }}" class="inline-flex items-center px-4 py-2.5 bg-slate-100 text-slate-600 rounded-xl font-semibold text-sm hover:bg-slate-200 transition-all duration-200">Reset</a>
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden">
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <div class="p-2 bg-gradient-to-br from-emerald-100 to-teal-100 text-emerald-600 rounded-xl">
                ...
                <table class="min-w-full">
                    <thead>
                        <tr class="bg-slate-50/70">
                            ...
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Kendaraan (Sumber)</th>
                            <th class="px-6 py-3.5 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Jenis BBM</th>
                            <th class="px-6 py-3.5 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider w-10"></th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Personel (Tujuan)</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div>
                                    <span class="text-sm font-medium text-slate-800">{{ $riwayat->created_at->format('d/m/Y') }}</span>
                                    <span class="text-xs text-slate-400 ml-1">{{ $riwayat->created_at->format('H:i') }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-center">
                                <span class="px-2.5 py-1 rounded-lg text-xs font-bold {{ ($riwayat->kendaraan->jenis_bbm ?? '') == 'Pertamina Dex' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700' }}">
                                    {{ $riwayat->kendaraan->jenis_bbm ?? '-' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center">
                                    <div class="w-6 h-6 rounded-full bg-gradient-to-b from-emerald-100 to-teal-100 border border-emerald-200 flex items-center justify-center">
                                        <svg class="w-3 h-3 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="w-7 h-7 rounded-lg bg-purple-100 flex items-center justify-center">
                                        <svg class="w-3.5 h-3.5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                                    </div>
                                    <div>
                            ...
                            <td class="px-6 py-4 text-right">
                                <span class="text-sm font-bold text-emerald-600">{{ number_format($riwayat->jumlah, 0, ',', '.') }} L</span>
                            </td>
                            <td class="px-6 py-4">
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-slate-400 text-sm mt-1">Coba ubah filter tanggal atau lakukan transfer terlebih dahulu</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden w-full md:w-1/2">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center gap-3">
                <div class="p-2 bg-gradient-to-br from-amber-100 to-orange-100 text-amber-600 rounded-xl">
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr>
                            <td class="py-3 flex items-center gap-2">
                                <span class="px-2.5 py-1 rounded-lg text-xs font-bold {{ $jenis == 'Pertamina Dex' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700' }}">
                                    {{ $jenis }}
                                </span>
                            </td>
                            <td class="py-3 text-right">
                                <span class="text-sm font-bold text-slate-900">{{ number_format($total, 0, ',', '.') }} Liter</span>
                            </td>
                        </tr>
                        @endforeach
                        @if($summary->isEmpty())
