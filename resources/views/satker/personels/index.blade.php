                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div class="flex gap-2">
                            <a href="{{ route('satker.personels.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg shadow transition-colors">
                                + Add Personel
                            </a>
                            <a href="{{ route('satker.personels.download-template') }}" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-lg shadow transition-colors flex items-center gap-2">
                        ...
                        <div class="flex flex-col md:flex-row gap-4 items-end md:items-center">
                            ...
                            <form action="{{ route('satker.personels.import') }}" method="POST" enctype="multipart/form-data" class="flex items-center gap-2">
                                @csrf
                                ...
                                <button type="submit" class="bg-violet-600 hover:bg-violet-700 text-white font-bold py-2 px-4 rounded-lg shadow transition-colors">
                                    Import
                                </button>
                            </form>
                        </div>
                    ...
                    @if(session('success'))
                        <div x-data="{ show: true }" x-show="show" x-transition class="mb-6 p-4 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-start gap-3 shadow-sm relative">
                    ...
                    @if(session('warning'))
                        <div x-data="{ show: true }" x-show="show" x-transition class="mb-6 p-4 rounded-2xl bg-amber-50 border border-amber-100 flex items-start gap-3 shadow-sm relative">
                    ...
                    @if(session('error'))
                        <div x-data="{ show: true }" x-show="show" x-transition class="mb-6 p-4 rounded-2xl bg-rose-50 border border-rose-100 flex items-start gap-3 shadow-sm relative">
                        ...
                        <table class="min-w-full divide-y divide-slate-200">
                            <thead>
                                <tr class="bg-slate-50/50">
                                    ...
                                    <th class="px-6 py-4 text-left text-[10px] font-bold text-slate-500 uppercase tracking-widest">Identitas Personel</th>
                                    <th class="px-6 py-4 text-left text-[10px] font-bold text-slate-500 uppercase tracking-widest">Jenis BBM</th>
                            ...
                            <tbody class="bg-white divide-y divide-slate-100">
                                ...
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center w-fit px-2.5 py-1 rounded-md text-[10px] font-bold border {{ $colorClass }}">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                        <div class="flex items-center justify-end gap-2">
                                            <a href="{{ route('satker.personels.print', $personel) }}" target="_blank" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Cetak Kartu">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z"></path></svg>
                                            </a>
                                            <a href="{{ route('satker.personels.edit', $personel) }}" class="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-all" title="Edit">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>
                                            @if($personel->saldo > 0)
                                                <span class="p-2 text-slate-200 cursor-not-allowed" title="Saldo masih {{ number_format($personel->saldo, 0, ',', '.') }} L">
                                            ...
                                            @else
                                                <form action="{{ route('satker.personels.destroy', $personel) }}" method="POST" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="p-2 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" onclick="return confirm('Hapus data personel {{ $personel->nama }}?')" title="Delete">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                    </button>
                                                </form>
                                            @endif
