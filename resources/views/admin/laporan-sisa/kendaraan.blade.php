    ...
    <div class="p-4 lg:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="mt-1 text-xs sm:text-sm font-medium text-slate-500">{{ $periode }}</p>
            </div>
            <div class="flex flex-row gap-2 w-full sm:w-auto">
                ...
                <a href="{{ route('admin.dashboard') }}"
                    class="flex-1 sm:flex-none inline-flex items-center justify-center px-5 py-2.5 bg-slate-100 text-slate-600 rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-slate-200 transition-all active:scale-95 gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="hidden lg:block bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="bg-slate-50 border-b border-slate-200 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">
                        <tr>
                            <th rowspan="2" class="px-4 py-4 border-r border-slate-100 w-16">NO</th>
                            <th rowspan="2" class="px-6 py-4 border-r border-slate-100">SATUAN KERJA</th>
                            <th colspan="2" class="px-6 py-3 border-b border-slate-100">SISA SALDO BBM (LITER)</th>
                        </tr>
                        <tr>
                            <th class="px-6 py-3 border-r border-slate-100">PERTAMAX</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @foreach($rows as $index => $row)
                            <tr class="hover:bg-slate-50/50 transition-all duration-200">
                                <td class="px-4 py-4 text-center font-bold text-slate-400 border-r border-slate-100">
                                    {{ $index + 1 }}</td>
                    ...
                    <tfoot class="bg-white text-slate-900 font-black text-center text-lg border-t-2 border-slate-100">
                        <tr>
                            <td colspan="2" class="px-6 py-5 text-right uppercase tracking-[0.2em] text-xs text-slate-500">TOTAL KESELURUHAN</td>
                            <td class="px-6 py-5 bg-emerald-50/30 border-l border-slate-100 font-black tracking-tighter text-emerald-600">
                                {{ number_format($totalPertamax, 0, ',', '.') }} <span class="text-[10px] text-emerald-600/50 ml-1">L</span>
                            </td>
                            <td class="px-6 py-5 bg-indigo-50/30 border-l border-slate-100 font-black tracking-tighter text-indigo-600">
        ...
        <div class="lg:hidden space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 relative overflow-hidden group">
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-xl bg-slate-100 flex items-center justify-center text-xs font-black text-slate-500 shadow-sm border border-slate-200">
                                {{ $index + 1 }}
                            </div>
                            <h3 class="text-sm font-black text-slate-800 uppercase leading-tight tracking-tight max-w-[200px]">
                                {{ $row['satker'] }}
                            </h3>
                        </div>
                    </div>
            ...
            <!-- Mobile Summary Card -->
            <div class="bg-white rounded-3xl p-6 text-slate-900 border border-slate-200 shadow-sm relative overflow-hidden">
                <div class="relative z-10">
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-4 border-b border-slate-100 pb-4">Total Keseluruhan</p>
                    <div class="space-y-4">
                        <div class="flex justify-between items-center">
                            <span class="text-xs font-bold text-slate-500">Pertamax</span>
                            <div class="flex items-baseline gap-1">
                        ...
                        <div class="flex justify-between items-center">
                            <span class="text-xs font-bold text-slate-500">Pertamina Dex</span>
                            <div class="flex items-baseline gap-1">
