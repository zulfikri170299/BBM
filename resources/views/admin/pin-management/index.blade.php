<x-app-layout>
    <div class="py-6 sm:py-10 bg-slate-50 min-h-screen" x-data="{ 
        tab: '{{ $tab }}',
        showModal: false,
        editingItem: null,
        newPin: '',
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h2 class="text-3xl font-black text-slate-800 tracking-tight">Manajemen PIN</h2>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                        ...
                        <div class="flex p-1.5 bg-slate-200/50 rounded-2xl w-fit">
                            <a href="{{ request()->fullUrlWithQuery(['tab' => 'personel']) }}"
                               class="px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ $tab === 'personel' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                                Personel
                            </a>
                            <a href="{{ request()->fullUrlWithQuery(['tab' => 'kendaraan']) }}"
                               class="px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ $tab === 'kendaraan' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700' }}">
                                Kendaraan
                            </a>
                        </div>
                        ...
                        <form action="{{ route('admin.pin-management.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
                            ...
                            <div class="relative min-w-[200px]">
                                <input type="text" name="search" value="{{ request('search') }}"
                                    class="w-full h-11 pl-10 pr-4 rounded-xl border-slate-200 bg-white text-xs font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all"
                            ...
                            <select name="satker_id" onchange="this.form.submit()"
                                class="tom-select h-11 min-w-[200px] rounded-xl border-slate-200 bg-white text-xs font-bold text-slate-700 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-50/50 border-b border-slate-100">
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Data {{ $tab === 'personel' ? 'Personel' : 'Kendaraan' }}</th>
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">Satuan Kerja</th>
                                <th class="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">PIN Saat Ini</th>
                            ...
                            @forelse($data as $item)
                                <tr class="hover:bg-slate-50/50 transition-all group">
                                    <td class="px-6 py-4">
                                        ...
                                        @else
                                            <div>
                                                <p class="text-sm font-black text-slate-800">{{ $item->no_polisi }}</p>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="inline-flex items-center px-3 py-1 bg-slate-100 rounded-lg text-xs font-mono font-black text-slate-700 tracking-widest">
                                            {{ $item->pin }}
                                        </div>
                                    </td>
        ...
        <div x-show="showModal" 
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-[2rem] shadow-2xl sm:my-8 sm:align-middle sm:max-w-md sm:w-full"
                     x-transition:enter="transition ease-out duration-300"
                    ...
                    <div class="px-6 pt-8 pb-6">
                        ...
                        <div class="text-center mb-8">
                            <h3 class="text-xl font-black text-slate-800 leading-tight">Ubah PIN Keamanan</h3>
                        ...
                        <div class="space-y-6">
                            <div>
                                ...
                                <input type="text" x-model="newPin" 
                                       ...
                                       class="w-full h-14 px-6 text-2xl font-mono font-black text-center tracking-[0.5em] border-2 border-slate-100 bg-slate-50/50 rounded-2xl focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all placeholder:text-slate-200"
                                       placeholder="••••••">
                            </div>

                            <div class="grid grid-cols-2 gap-3">
                                <button @click="showModal = false"
                                        class="h-12 px-6 bg-slate-100 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
                                    Batal
                                </button>
                                <button @click="savePin()"
                                        :disabled="loading || newPin.length !== 6"
                                        class="h-12 px-6 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 disabled:opacity-50 flex items-center justify-center gap-2">
