<x-app-layout>
    ...
    <div class="fixed top-24 right-6 z-[100] w-full max-w-sm space-y-4 pointer-events-none">
        @if(session('success'))
            <div class="bg-white border-l-4 border-emerald-500 shadow-xl rounded-r-xl p-4 flex items-start transform transition-all duration-300 hover:scale-105 pointer-events-auto animate-bounce-in-right">
                ...
                <button type="button" onclick="this.parentElement.remove()" class="ml-4 flex-shrink-0 text-slate-400 hover:text-slate-500 focus:outline-none">
        ...
        @if(session('error'))
            <div class="bg-white border-l-4 border-rose-500 shadow-xl rounded-r-xl p-4 flex items-start transform transition-all duration-300 hover:scale-105 pointer-events-auto animate-bounce-in-right">
        ...
        @if ($errors->any())
            <div class="bg-white border-l-4 border-rose-500 shadow-xl rounded-r-xl p-4 flex items-start transform transition-all duration-300 hover:scale-105 pointer-events-auto animate-bounce-in-right">
                ...
                <div class="ml-3 w-0 flex-1 pt-0.5">
                    <p class="text-sm font-bold text-slate-800">Terjadi Kesalahan</p>
                    <ul class="list-disc list-inside text-xs text-slate-500 mt-1">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden sticky top-8">
                    ...
                    <form action="{{ route('admin.stok.store') }}" method="POST" class="p-4 sm:p-6 space-y-4" autocomplete="off">
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-slate-700 mb-2">Password Konfirmasi (Top-up)</label>
                            <input type="password" name="topup_security_code" class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all outline-none" placeholder="Masukkan password top-up..." required autocomplete="off" readonly onfocus="this.removeAttribute('readonly');">
