<x-app-layout>
    <div class="p-2 sm:p-6 lg:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-white tracking-tight">Riwayat Pengisian BBM</h1>
        ...
        <div class="bg-slate-900 border border-white/5 rounded-2xl border border-white/10 shadow-sm p-4 sm:p-5">
            <form method="GET" action="{{ route('admin.riwayat.index') }}" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
                    ...
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Cari</label>
                        <input type="text" placeholder="Nopol/Nama..." name="search" value="{{ request('search') }}"
                            class="w-full h-11 px-4 border border-white/10 rounded-xl text-xs font-bold text-slate-300 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all bg-slate-800/50">
                    </div>
