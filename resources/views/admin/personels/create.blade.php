<x-app-layout>
    <div class="py-6 sm:py-10 bg-slate-50 dark:bg-slate-800/50 min-h-screen px-2 sm:px-6 lg:px-8">
        <div class="max-w-lg mx-auto px-4 sm:px-6">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-[1.5rem] shadow-xl shadow-slate-200/50 border border-slate-200 dark:border-white/5 overflow-hidden">
                <!-- Header Section -->
                <div class="p-5 sm:p-6 bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-white/5">
                    <h2 class="text-xl font-black text-slate-800 dark:text-slate-200 tracking-tight">Tambah Personel</h2>
                    <p class="text-[10px] text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider mt-1">Input data personel secara ringkas</p>
                </div>
                ...
                <div class="p-5 sm:p-6">
                    <form action="{{ route('admin.personels.store') }}" method="POST" class="space-y-4">
                        ...
                        <!-- Satker -->
                        <div>
                            <label class="block text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Satuan Kerja</label>
                            <select name="satker_id"
                                ...
                                @foreach($satkers as $satker)
                                    <option class="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" value="{{ $satker->id }}" {{ old('satker_id') == $satker->id ? 'selected' : '' }}>
                                        {{ $satker->nama_satker }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <!-- Nama -->
                        <div>
                            <label class="block text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Nama Lengkap</label>
                        ...
                        <!-- NRP -->
                        <div>
                            <label class="block text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1.5 ml-1">NRP / NIP</label>
                        ...
                        <div>
                            ...
                            <select name="jenis_bbm"
                                ...
                                <option class="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" value="Pertamax" {{ old('jenis_bbm', 'Pertamax') == 'Pertamax' ? 'selected' : '' }}>Pertamax</option>
            ...
            <p class="mt-6 text-center text-slate-600 dark:text-slate-400 text-[9px] font-black uppercase tracking-widest">Biro Logistik &copy; 2026</p>
        </div>
    </div>
</x-app-layout>