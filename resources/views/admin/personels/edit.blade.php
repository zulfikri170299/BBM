            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="flex items-center justify-between mb-6">
                        <h2 class="text-2xl font-bold text-slate-800">Edit Personel</h2>
                    ...
                    <form action="{{ route('admin.personels.update', $personel) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="jenis_bbm"
                                    ...
                                    @foreach(['Pertalite', 'Pertamax', 'Solar', 'Pertamina Dex'] as $bbm)
                                        <option value="{{ $bbm }}" {{ old('jenis_bbm', $personel->jenis_bbm) == $bbm ? 'selected' : '' }}>{{ $bbm }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                <label class="block text-sm font-bold text-slate-700 mb-2">Saldo (Liter)</label>
                                <input type="number" step="0.1" name="saldo"
                                    value="{{ old('saldo', $personel->saldo) }}"
                                    class="w-full rounded-lg border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 shadow-sm"
                                    required>
                        ...
                        <div class="mt-8 flex justify-end gap-3">
                            <button type="submit"
                                class="px-6 py-2.5 bg-indigo-600 text-white font-bold rounded-lg hover:bg-indigo-700 transition-colors shadow-sm">Update
                                Personel</button>
                        </div>
                    </form>
                </div>
