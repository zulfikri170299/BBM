<x-app-layout>
    <div class="p-6 lg:p-8 space-y-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="mt-1 text-slate-500">Pantau status aktif, lokasi terakhir, dan riwayat perubahan akun pengguna.</p>
            </div>
        </div>

        <div class="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
            <form action="{{ route('admin.users.monitoring') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                <!-- Search -->
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Cari User</label>
                    <div class="relative">
                ...
                <!-- Satker -->
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Filter Satker</label>
                    <select name="satker_id" class="w-full py-2 bg-slate-50 border-slate-200 rounded-xl text-sm focus:ring-indigo-500 focus:border-indigo-500 transition-all">
                ...
                <!-- Tanggal -->
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Filter Tanggal Aktif</label>
                ...
                <div class="flex items-end gap-2">
                    <button type="submit" class="flex-1 bg-indigo-600 text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-indigo-700 transition-all shadow-sm shadow-indigo-100">
                        Terapkan
                    </button>
                    @if(request()->anyFilled(['search', 'satker_id', 'tanggal']))
                        <a href="{{ route('admin.users.monitoring') }}" class="px-4 py-2 bg-slate-100 text-slate-600 rounded-xl text-sm font-bold hover:bg-slate-200 transition-all text-center">
                            Reset
                        </a>
                    @endif
                </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <!-- Bulk Actions -->
            <div id="bulkActions" class="hidden px-6 py-3 border-b border-slate-100 flex items-center justify-between bg-indigo-50/50 animate-in fade-in duration-300">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        <button type="button" onclick="submitBulk('active')" class="px-3 py-1.5 bg-emerald-600 text-white rounded-lg text-xs font-bold hover:bg-emerald-700 transition shadow-sm">Aktifkan Terpilih</button>
                        <button type="button" onclick="submitBulk('inactive')" class="px-3 py-1.5 bg-rose-600 text-white rounded-lg text-xs font-bold hover:bg-rose-700 transition shadow-sm">Nonaktifkan Terpilih</button>
                    </div>
                </div>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Satker & Role</th>
                            <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Status Terakhir</th>
                            <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Lokasi Terakhir</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-xs font-semibold text-slate-700">{{ $user->satker->nama_satker ?? 'Pusat' }}</p>
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if($user->last_activity_at)
                                    <p class="text-xs font-medium text-slate-700">{{ $user->last_activity_at->diffForHumans() }}</p>
                                    <p class="text-[10px] text-slate-400">{{ $user->last_activity_at->format('d/m H:i') }} WIB</p>
                                @else
                            ...
                            <td class="px-6 py-4">
                                @if($user->last_latitude && $user->last_longitude)
                                    <a href="https://www.google.com/maps?q={{ $user->last_latitude }},{{ $user->last_longitude }}" target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-sky-50 text-sky-600 rounded-lg text-xs font-bold hover:bg-sky-100 transition-colors">
                                ...
                                <div class="flex items-center justify-center gap-2">
                                    @if(auth()->id() !== $user->id)
                                        <form action="{{ route('admin.users.toggle', $user) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="p-1.5 {{ $user->is_active ? 'text-rose-400 hover:text-rose-600' : 'text-emerald-400 hover:text-emerald-600' }} transition-colors" title="{{ $user->is_active ? 'Nonaktifkan Akun' : 'Aktifkan Akun' }}">
                                                @if($user->is_active)
                                    ...
                                    @endif
                                    <button onclick="showLogs({{ $user->id }}, '{{ $user->name }}')" class="p-1.5 text-slate-400 hover:text-indigo-600 transition-colors" title="Lihat Detail Aktivitas">
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-slate-400 italic">Tidak ada data user.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div id="logModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-2xl shadow-xl sm:my-8 sm:align-middle sm:max-w-xl sm:w-full">
