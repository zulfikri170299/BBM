                    ...
                    <form action="{{ route('admin.users.update', $user) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="role" id="role" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
                                ...
                                <option value="admin_satker" {{ $user->role == 'admin_satker' ? 'selected' : '' }}>Admin Satker</option>
                                <option value="petugas_bbm" {{ $user->role == 'petugas_bbm' ? 'selected' : '' }}>Petugas BBM</option>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="satker_id" id="satker_id" class="shadow border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                <option value="">Tidak Ada</option>
                                @foreach($satkers as $satker)
                                    <option value="{{ $satker->id }}" {{ $user->satker_id == $satker->id ? 'selected' : '' }}>{{ $satker->nama_satker }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                        ...
                        <div class="flex items-center justify-between">
                            <button type="submit" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">Perbarui Pengguna</button>
                        </div>
                    </form>
                </div>
            </div>
