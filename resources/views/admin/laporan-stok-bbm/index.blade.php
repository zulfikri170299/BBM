    ...
    <div class="p-4 sm:p-6 lg:p-8 space-y-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-3xl font-black text-slate-900 tracking-tight">Data BBM Pada Tangki</h1>
                    <p class="text-slate-500 text-sm font-medium mt-1 uppercase tracking-widest flex items-center gap-2">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('admin.laporan-stok-bbm.print', request()->all()) }}" target="_blank"
                    class="px-6 py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl font-bold shadow-lg shadow-rose-200 transition-all active:scale-[0.98] flex items-center gap-2 group">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50/50">
                            <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">Waktu Input & Petugas</th>
                            <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">Stok Awal Fisik</th>
                            <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">Pemakaian Sistem</th>
                            <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">Sisa Stok Akhir</th>
                            @if(in_array(auth()->user()->role, ['super_admin', 'kasubbag']))
                            <th class="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100 text-center">Aksi</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse($syncs as $index => $sync)
                            <!-- Pertamax Row -->
                            <tr class="group hover:bg-indigo-50/30 transition-colors {{ $index % 2 == 0 ? 'bg-white' : 'bg-slate-50/20' }}">
                                <td class="px-8 py-6" rowspan="2">
                                    <div class="flex items-start gap-4">
                                        ...
                                        <div>
                                            <div class="text-sm font-black text-slate-800">{{ $sync->created_at->format('H:i') }} <span class="text-slate-400 font-bold">WITA</span></div>
                                ...
                                <td class="px-8 py-6 text-center" rowspan="2">
                                    <div class="flex flex-col gap-2 items-center">
                                        <a href="{{ route('admin.laporan-stok-bbm.edit', $sync->id) }}" 
                                            class="w-full max-w-[100px] py-2 bg-white border border-slate-200 text-slate-600 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 rounded-xl font-black text-[10px] transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                        ...
                                        <form action="{{ route('admin.laporan-stok-bbm.destroy', $sync->id) }}" method="POST" class="w-full max-w-[100px]">
                                            ...
                                            <button type="submit" 
                                                ...
                                                class="w-full py-2 bg-white border border-slate-200 text-rose-600 hover:bg-rose-600 hover:text-white hover:border-rose-600 rounded-xl font-black text-[10px] transition-all flex items-center justify-center gap-1.5 shadow-sm">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <!-- Dex Row -->
                            <tr class="group hover:bg-indigo-50/30 transition-colors {{ $index % 2 == 0 ? 'bg-white' : 'bg-slate-50/20' }} border-b border-slate-100">
                                <td class="px-8 py-6 border-l border-slate-100">
                                    <div class="flex items-center justify-between">
