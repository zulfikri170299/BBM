        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6 p-4 bg-slate-50 rounded-2xl border border-slate-200 shadow-sm overflow-x-auto">
                        <form action="{{ route('admin.nominatif.index') }}" method="GET" class="flex items-end gap-3 min-w-max">
                            <div class="space-y-1">
                                ...
                                <select name="tahun" class="tom-select w-28 px-3 py-1.5 rounded-xl border-slate-200 text-xs font-bold">
                            ...
                            <div class="space-y-1">
                                ...
                                <select name="bulan" class="tom-select w-40 px-3 py-1.5 rounded-xl border-slate-200 text-xs font-bold">
                            ...
                            <div class="space-y-1">
                                ...
                                <input type="date" name="start_date" value="{{ request('start_date', $startDate) }}" 
                                    class="w-36 px-3 py-1.5 rounded-xl border-slate-200 text-xs font-bold">
                            </div>
                            ...
                            <div class="flex gap-1.5 ml-2">
                                ...
                                @if(request('start_date') || request('end_date'))
                                    <a href="{{ route('admin.nominatif.index') }}" class="px-4 py-1.5 bg-slate-200 text-slate-600 text-xs font-bold rounded-xl hover:bg-slate-300 transition-all active:scale-95">Reset</a>
                                @endif
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Satker (Uraian)</th>
                                    <th class="px-6 py-3 text-right text-xs font-bold text-gray-500 uppercase">Pertamax (L)</th>
                                    <th scope="col" class="px-6 py-3 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Pertamina Dex (L)</th>
                                    <th scope="col" class="px-6 py-3 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Satuan</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                @forelse($logs as $index => $log)
                                    ...
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 font-bold text-gray-800">{{ ucwords(strtolower($log->satker->nama_satker)) }}</td>
                                        <td class="px-6 py-4 text-right font-black text-indigo-600">{{ number_format($log->total_pertamax, 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 text-right font-black text-rose-600">{{ number_format($log->total_dex, 0, ',', '.') }}</td>
                                        <td class="px-6 py-4 text-center text-xs text-gray-400">Liter</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-10 text-center text-gray-500">Tidak ada data untuk periode ini.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            @if(count($logs) > 0)
                            <tfoot class="bg-slate-50 font-bold border-t-2 border-slate-200">
                                <tr>
                                    <td class="px-6 py-4 text-center" colspan="3">JUMLAH</td>
                                    <td class="px-6 py-4 text-right font-black text-indigo-700 leading-none">{{ number_format($total_p, 0, ',', '.') }}</td>
                                    <td class="px-6 py-4 text-right font-black text-rose-700 leading-none">{{ number_format($total_d, 0, ',', '.') }}</td>
