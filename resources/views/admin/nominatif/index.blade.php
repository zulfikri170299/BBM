                ...
                <div class="p-6 text-slate-800 dark:text-white">
                    <div class="flex justify-between items-center mb-6">
                        <div>
                            ...
                            <p class="text-slate-600 dark:text-slate-400">Laporan nominatif penerimaan BBM per Satker.</p>
                        </div>
                    </div>
                    ...
                    <div class="mb-6 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-x-auto">
                        <form action="{{ route('admin.nominatif.index') }}" method="GET" class="flex items-end gap-3 min-w-max">
                            <div class="space-y-1">
                                <label class="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase ml-1">Tahun</label>
                                <select name="tahun" class="tom-select w-28">
                                    @php $currentYear = date('Y'); @endphp
                                    @for($i = $currentYear; $i >= $currentYear - 5; $i--)
                                        <option class="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" value="{{ $i }}" {{ request('tahun', $tahun) == $i ? 'selected' : '' }}>{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
                            <div class="space-y-1">
                                <label class="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase ml-1">Bulan</label>
                                <select name="bulan" class="tom-select w-40">
                                    @foreach(range(1, 12) as $m)
                                        <option class="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" value="{{ $m }}" {{ request('bulan', $bulan) == $m ? 'selected' : '' }}>
                            ...
                            <div class="space-y-1">
                                <label class="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase ml-1">Mulai</label>
                                <input type="date" name="start_date" value="{{ request('start_date', $startDate) }}" 
                                    class="bg-white dark:bg-slate-900 dark:text-white w-36 px-3 py-1.5 rounded-xl border-slate-200 dark:border-white/10 text-xs font-bold">
                            </div>

                            <div class="space-y-1">
                                <label class="text-[10px] font-bold text-slate-600 dark:text-slate-400 uppercase ml-1">Sampai</label>
                                <input type="date" name="end_date" value="{{ request('end_date', $endDate) }}" 
                                    class="bg-white dark:bg-slate-900 dark:text-white w-36 px-3 py-1.5 rounded-xl border-slate-200 dark:border-white/10 text-xs font-bold">
                            </div>

                            <div class="flex gap-1.5 ml-2">
                                <button type="submit" class="px-4 py-1.5 bg-slate-50 text-slate-800 dark:text-white text-xs font-bold rounded-xl hover:bg-white dark:bg-slate-900 transition-all active:scale-95">Tampilkan</button>
                                
                                @if(request('start_date') || request('end_date'))
                                    <a href="{{ route('admin.nominatif.index') }}" class="px-4 py-1.5 bg-slate-200 text-slate-600 dark:text-slate-400 text-xs font-bold rounded-xl hover:bg-slate-300 transition-all active:scale-95">Reset</a>
                                @endif
                    ...
                    <div class="overflow-x-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-xl shadow-sm border border-slate-200 dark:border-white/5">
                        <table class="min-w-full divide-y divide-slate-300 dark:divide-white/10">
                            <thead class="bg-slate-50 dark:bg-slate-800/50">
                                <tr>
                                    <th class="px-4 py-3 text-left text-xs font-bold text-slate-600 dark:text-slate-400 uppercase w-12">No</th>
                                    <th class="px-4 py-3 text-left text-xs font-bold text-slate-600 dark:text-slate-400 uppercase w-32">Tanggal</th>
                                    <th class="px-4 py-3 text-left text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">Satker (Uraian)</th>
                                    <th class="px-4 py-3 text-right text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">Pertamax (L)</th>
                                    <th scope="col" class="px-4 py-3 text-right text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Pertamina Dex (L)</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Satuan</th>
                                    <th scope="col" class="px-4 py-3 text-center text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider">Aksi</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 divide-y divide-slate-300 dark:divide-white/10">
                                ...
                                @forelse($logs as $index => $log)
                                    ...
                                    <tr class="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                                        ...
                                        <td class="px-4 py-3 font-bold text-slate-800 dark:text-slate-200">{{ ucwords(strtolower($log->satker->nama_satker)) }}</td>
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-10 text-center text-slate-600 dark:text-slate-400">Tidak ada data untuk periode ini.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                            @if(count($logs) > 0)
                            <tfoot class="bg-slate-50 dark:bg-slate-800/50 font-bold border-t-2 border-slate-200 dark:border-white/10">
                                <tr>
