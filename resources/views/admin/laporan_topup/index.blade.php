<x-app-layout>
    <div class="p-2 sm:p-6 lg:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-slate-800 dark:text-white tracking-tight">Laporan Top Up</h1>
                <p class="mt-1 text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-400">Riwayat pengisian saldo kendaraan.</p>
            </div>
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm p-4 sm:p-5">
            <form action="{{ route('admin.laporan-topup.index') }}" method="GET" class="space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        <label class="block text-[10px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Mulai Tanggal</label>
                        <input type="date" name="start_date" value="{{ request('start_date') }}"
                            class="flatpickr w-full h-11 px-4 bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-white/10 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all">
                    </div>
                    <div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
            ...
            <div class="bg-white dark:bg-slate-900 dark:text-white px-4 py-3 border-b border-slate-200 dark:border-white/5 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h3 class="font-semibold text-slate-800 dark:text-slate-200">Riwayat Top Up</h3>
                        <p class="text-xs text-slate-600 dark:text-slate-400">Total {{ $riwayats->total() }} catatan</p>
                    </div>
                </div>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-white/5">
                        @forelse($riwayats as $riwayat)
                            <tr class="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors">
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ $riwayat->created_at->setTimezone('Asia/Makassar')->format('d/m/Y H:i') }}
                                        WITA</span>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-6 h-6 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-400">
                                            {{ substr($riwayat->user->name ?? '?', 0, 1) }}
                                        </div>
                                        <span class="text-xs text-slate-700 dark:text-slate-300">{{ $riwayat->user->name ?? '-' }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3">
                                    <span
                                        class="text-xs font-semibold text-slate-800 dark:text-slate-200">{{ $riwayat->kendaraan->no_polisi ?? '-' }}</span>
                                </td>
                                <td class="px-4 py-3 text-center">
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-slate-600 dark:text-slate-400 font-medium">Belum ada riwayat top up</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
        ...
        <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden w-full md:w-1/2">
            <div class="bg-white dark:bg-slate-900 dark:text-white px-4 py-3 border-b border-slate-200 dark:border-white/5">
                <h3 class="font-semibold text-slate-800 dark:text-slate-200">Total Keseluruhan per Jenis BBM</h3>
                ...
                <table class="w-full">
                    <tbody class="divide-y divide-slate-200 dark:divide-white/5">
                        @foreach($summary as $jenis => $total)
                            <tr>
                                <td class="py-3 text-sm font-medium text-slate-600 dark:text-slate-400">{{ $jenis }}</td>
                                <td class="py-3 text-sm font-bold text-slate-800 dark:text-white text-right">
                        ...
                        @if($summary->isEmpty())
                            <tr>
                                <td colspan="2" class="py-3 text-sm text-center text-slate-600 dark:text-slate-400">Tidak ada data</td>
                            </tr>
                        @endif
                    </tbody>
                </table>
