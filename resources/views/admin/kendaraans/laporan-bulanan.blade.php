    ...
    <div class="p-4 lg:p-6 space-y-4">
        ...
        <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-lg border border-slate-200 dark:border-white/10 p-4">
            <form action="{{ route('admin.kendaraans.laporan-bulanan') }}" method="GET"
                ...
                <div>
                    <label class="text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1 block">Satker</label>
                    <select name="satker_id" class="tom-select w-56 sm:w-64" required>
                        @foreach($satkers as $s)
                            <option class="bg-white text-slate-900 dark:bg-slate-900 dark:text-white" value="{{ $s->id }}" {{ $satker->id == $s->id ? 'selected' : '' }}>
                                {{ $s->nama_satker }}
                            </option>
                        @endforeach
                    </select>
