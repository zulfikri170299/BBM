        ...
        <div x-cloak x-data="{
            ...
            <div x-show="showTopup" style="display: none;" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="bg-white rounded-2xl shadow-2xl w-full max-w-xl max-h-[95vh] flex flex-col overflow-hidden"
                    ...
                    <!-- Modal Body & Form -->
                    <form :action="'/admin/kendaraans/' + topupId + '/topup'" method="POST" class="flex flex-col flex-1 overflow-hidden">
                        @csrf
                                    ...
                                    <div>
                                        ...
                                        <div class="relative" @click.outside="satkerOpen = false">
                                            <div @click="satkerOpen = !satkerOpen; $nextTick(() => { if(satkerOpen) $refs.satkerInput.focus() })"
                                                ...
                                                <span x-text="satkerLabel || '-- Pilih Satker --'" :class="satkerLabel ? 'text-slate-800' : 'text-slate-400'"></span>
                                                <svg class="w-4 h-4 text-slate-400 transition-transform" :class="satkerOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                            </div>
                                            <div x-show="satkerOpen" x-transition.opacity.duration.150ms class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden" style="display:none;">
                                                <div class="p-2 border-b border-slate-100">
                                                    <div class="relative">
                                                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                                                        <input x-ref="satkerInput" x-model="satkerSearch" type="text" placeholder="Cari Satker..." class="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                                                    </div>
                                                </div>
                                                <div class="max-h-48 overflow-y-auto">
                                                    <template x-for="s in filteredSatkers" :key="s.id">
                                                        <div @click="selectSatker(s)" class="px-4 py-2.5 text-xs sm:text-sm text-slate-700 hover:bg-emerald-50 cursor-pointer flex items-center justify-between transition-colors" :class="selectedSatkerId == s.id ? 'bg-emerald-50 text-emerald-700 font-semibold' : ''">
                                                            <span x-text="s.nama"></span>
                                                            <svg x-show="selectedSatkerId == s.id" class="w-4 h-4 text-emerald-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>
                                                        </div>
                                                    </template>
                                                    <div x-show="filteredSatkers.length === 0" class="px-4 py-3 text-sm text-slate-400 text-center">Tidak ditemukan</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div x-show="selectedSatkerId">
                                        ...
                                        <div class="relative" @click.outside="kendaraanOpen = false">
                                            <div @click="kendaraanOpen = !kendaraanOpen; $nextTick(() => { if(kendaraanOpen) $refs.kendaraanInputManual.focus() })"
                                                ...
                                                <span x-text="kendaraanLabel || '-- Pilih Kendaraan --'" :class="kendaraanLabel ? 'text-slate-800' : 'text-slate-400'"></span>
                                                <svg class="w-4 h-4 text-slate-400 transition-transform" :class="kendaraanOpen ? 'rotate-180' : ''" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                            </div>
                                            <div x-show="kendaraanOpen" x-transition.opacity.duration.150ms class="absolute z-50 mt-1 w-full bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden" style="display:none;">
                                                <div class="p-2 border-b border-slate-100">
                                                    <div class="relative">
                                                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                                                        <input x-ref="kendaraanInputManual" x-model="kendaraanSearch" type="text" placeholder="Cari nopol..." class="w-full pl-9 pr-3 py-2 text-sm border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                                                    </div>
                                                </div>
                                                <div class="max-h-48 overflow-y-auto">
                                                    <template x-for="k in filteredKendaraans" :key="k.id">
                                                        <div @click="selectKendaraanManual(k)" class="px-4 py-2.5 text-xs sm:text-sm text-slate-700 hover:bg-emerald-50 cursor-pointer flex items-center justify-between transition-colors" :class="topupId == k.id ? 'bg-emerald-50 text-emerald-700 font-semibold' : ''">
                                                            <span x-text="k.nopol + ' (' + k.saldo + ' L)'"></span>
                                                            <svg x-show="topupId == k.id" class="w-4 h-4 text-emerald-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path></svg>
                                                        </div>
                                                    </template>
                                                    <div x-show="filteredKendaraans.length === 0" class="px-4 py-3 text-sm text-slate-400 text-center">Tidak ditemukan</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
