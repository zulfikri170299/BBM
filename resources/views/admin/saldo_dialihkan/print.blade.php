...
<html>
<head>
    <title>Laporan Saldo Dialihkan (Admin)</title>
    <style>
    ...
    <div class="header">
        @php
            ...
            if(request('satker_id')){
                $satkerModel = \App\Models\Satker::find(request('satker_id'));
                if($satkerModel) $satkerName = strtoupper($satkerModel->nama_satker);
            }
        @endphp
        <h1>LAPORAN SALDO BBM DI ALIHKAN OLEH {{ $satkerName }}</h1>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="20%">KENDARAAN / NOPOL</th>
        ...
        <tbody>
            ...
            @if($riwayat->isEmpty())
            <tr>
                <td colspan="6" class="text-center" style="padding: 20px;">Tidak ada riwayat saldo yang dialihkan.</td>
            </tr>
            @endif
        </tbody>
    </table>
