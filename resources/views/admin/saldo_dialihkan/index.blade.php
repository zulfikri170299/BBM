<x-app-layout>
    <div class="max-w-7xl mx-auto p-2 sm:p-6 lg:p-8 space-y-6 px-2 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/10 pb-5">
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-white tracking-tight">Saldo Yang di Alihkan (Super Admin)</h1>
                <p class="mt-1 text-xs sm:text-sm font-medium text-slate-400">Laporan pemotongan saldo kendaraan dari seluruh Satker.</p>
            </div>
        </div>
            ...
            <form action="{{ route('admin.saldo-dialihkan.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
                ...
                <div class="flex items-center gap-2 w-full sm:w-auto mt-4 sm:mt-0">
                    ...
                    @if(request()->has('start_date') || request()->has('satker_id'))
                        <a href="{{ route('admin.saldo-dialihkan.index') }}" class="flex-1 sm:flex-none inline-flex justify-center items-center px-4 py-2.5 bg-slate-900 border border-white/5 hover:bg-slate-800/50 text-slate-300 text-sm font-bold rounded-xl border border-white/10 transition-all shadow-sm active:scale-95 gap-2">
                            Reset
                        </a>
                    @endif
