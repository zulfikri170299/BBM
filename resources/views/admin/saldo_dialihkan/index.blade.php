<x-app-layout>
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-5">
            <div>
                <h1 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Saldo Yang di Alihkan (Super Admin)</h1>
                <p class="mt-1 text-xs sm:text-sm font-medium text-slate-500">Laporan pemotongan saldo kendaraan dari seluruh Satker.</p>
            </div>
        </div>
            ...
            <form action="{{ route('admin.saldo-dialihkan.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4 items-end">
                <div class="flex-1 w-full flex flex-col sm:flex-row gap-4">
                    <div class="w-full sm:w-1/3">
                        <label class="block text-xs font-bold text-slate-700 mb-1.5 uppercase tracking-wider">Satker</label>
                        <select name="satker_id" class="tom-select w-full text-sm font-medium bg-slate-50 border border-slate-200 text-slate-900 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 transition-all outline-none">
                ...
                <div class="flex items-center gap-2 w-full sm:w-auto mt-4 sm:mt-0">
                    ...
                    @if(request()->has('start_date') || request()->has('satker_id'))
                        <a href="{{ route('admin.saldo-dialihkan.index') }}" class="flex-1 sm:flex-none inline-flex justify-center items-center px-4 py-2.5 bg-white hover:bg-slate-50 text-slate-700 text-sm font-bold rounded-xl border border-slate-200 transition-all shadow-sm active:scale-95 gap-2">
                            Reset
                        </a>
                    @endif
                ...
                <table class="w-full text-sm text-left">
                    <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-6 py-4 font-black">SATKER</th>
