<x-app-layout>
    <div class="p-2 sm:p-6 lg:p-8 space-y-6">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white">Pesan Siaran (Broadcast)</h1>
                <p class="mt-1 text-slate-600 dark:text-slate-400">Kirimkan notifikasi penting ke seluruh pengguna aplikasi.</p>
            </div>
        </div>
        ...
        <div class="max-w-3xl">
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-white/10 shadow-sm overflow-hidden">
                <div class="p-6 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-slate-800/50">
                ...
                <form action="{{ route('admin.broadcast.store') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Judul Informasi</label>
                        <input type="text" name="title" required
                            placeholder="Contoh: Pemeliharaan Sistem / Update Harga BBM"
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-white/10 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm">
                        @error('title') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Isi Pesan</label>
                        <textarea name="message" rows="5" required
                            placeholder="Tuliskan informasi detail yang ingin disampaikan..."
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-white/10 rounded-xl focus:ring-indigo-500 focus:border-indigo-500 transition-all text-sm"></textarea>
                        @error('message') <p class="mt-1 text-xs text-rose-500">{{ $message }}</p> @enderror
                        <p class="mt-2 text-xs text-slate-600 dark:text-slate-400">Pesan ini akan muncul di menu notifikasi (lonceng) pada
