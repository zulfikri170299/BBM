<x-app-layout>
    <div x-data="transaksi()" @@start-scan.window="startScanner()" @@stop-scan.window="stopScanner()" class="max-w-sm mx-auto p-2 sm:p-4 pb-12">
        ...
        <div class="text-center mb-3">
            <h1 class="text-xl font-bold text-slate-800 dark:text-white tracking-wide">TRANSAKSI BBM</h1>
            <p class="text-[8px] font-bold text-slate-600 dark:text-slate-400 uppercase tracking-widest leading-none mt-0.5" x-text="step === 'search' ? 'IDENTIFIKASI' : 'KONFIRMASI'"></p>
        </div>
        ...
        <div x-show="step === 'search'" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="space-y-2.5">
            <!-- Tabs -->
            <div class="flex bg-white dark:bg-slate-900/50 p-1 rounded-xl border border-slate-200 dark:border-white/5">
                <button @@click="tab = 'barcode'; stopScanner();" 
                    :class="tab === 'barcode' ? 'bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300'"
                    class="flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all">Barcode</button>
                <button @@click="tab = 'manual'; stopScanner();" 
                    :class="tab === 'manual' ? 'bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:text-slate-300'"
                    class="flex-1 py-2 text-[10px] font-bold uppercase rounded-lg transition-all">{{ $personelAccessControl == '1' ? 'NOPOL / NRP' : 'NOPOL' }}</button>
            </div>

            <!-- Card Container -->
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-4 shadow-xl shadow-indigo-500/10 relative">
                <!-- Barcode -->
                <div x-show="tab === 'barcode'" class="space-y-3 flex flex-col items-center">
                    <div class="w-full max-w-[260px] aspect-square bg-slate-50 dark:bg-slate-800/50 rounded-xl relative overflow-hidden border-2 border-slate-200 dark:border-white/5">
                        <div id="reader" class="w-full h-full"></div>
                        <div id="scanner-placeholder" x-show="!isScannerActive || isLoadingScanner" 
                             class="absolute inset-0 flex flex-col items-center justify-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5/90">
                            ...
                            <div x-show="!isLoadingScanner" class="relative group">
                                <div class="relative w-36 h-36 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-3xl border border-slate-200 dark:border-white/10 shadow-sm flex items-center justify-center overflow-hidden">
                                    <svg class="w-20 h-20 text-indigo-600 dark:text-indigo-300/80" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <p class="text-slate-600 dark:text-slate-400 text-[8px] font-black uppercase tracking-[0.2em] mt-3" x-text="isLoadingScanner ? 'INIT...' : 'SIAP SCAN'"></p>
                        </div>
                        <!-- Square Scan Overlay -->
                    ...
                    <button type="button" @@click="isScannerActive ? stopScanner() : startScanner()" 
                        ...
                        :class="isScannerActive ? 'bg-white dark:bg-slate-900 text-white' : 'bg-indigo-600 hover:bg-indigo-700 text-white' "
                ...
                <div x-show="tab !== 'barcode'" class="space-y-3 py-1" x-cloak>
                    ...
                    <input type="text" x-model="manualValue" @@keyup.enter="checkData(tab, manualValue)"
                        class="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-white/5 rounded-xl text-2xl text-center font-black text-slate-800 dark:text-white uppercase tracking-widest outline-none"
        ...
        <div x-show="step === 'form'" x-transition:enter="transition duration-300" class="space-y-2.5" x-cloak>
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-lg">
                <div :class="type === 'kendaraan' ? 'bg-indigo-600' : 'bg-rose-600'" class="px-4 py-3.5 text-slate-800 dark:text-white flex justify-between items-end">
                    <div>
                ...
                <div class="px-4 py-2.5 grid grid-cols-2 gap-3 bg-slate-50 dark:bg-slate-800/50">
                    <span class="text-[11px] font-bold text-slate-700 dark:text-slate-300 truncate" x-text="type === 'kendaraan' ? target.jenis_kendaraan : target.nama"></span>
            ...
            <form action="{{ route('petugas.transaksi.process') }}" method="POST" class="space-y-2.5" autocomplete="off">
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-2xl p-4 space-y-3.5 shadow-lg shadow-black/20">
                    <div>
                        <label class="block text-[8px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">JUMLAH LITER</label>
                        <div class="relative">
                            <input type="number" name="liter" step="1" min="1" :max="target.saldo" required x-model="liter"
                                class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-white/5 rounded-xl text-3xl font-black text-indigo-600 outline-none"
                                autocomplete="off" placeholder="0.0">
                        </div>
                    </div>
                    <div>
                        <label class="block text-[8px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">NAMA DRIVER</label>
                        <input type="text" name="nama_driver" required
                            class="w-full px-4 py-3.5 bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-white/5 rounded-xl text-xs font-bold text-slate-800 dark:text-slate-200 outline-none"
                            autocomplete="off" spellcheck="false" placeholder="Siapa pembawa?">
                    </div>
                    <div>
                        <label class="block text-[8px] font-black text-slate-600 dark:text-slate-400 uppercase tracking-widest mb-1">PIN OTORISASI</label>
                        ...
                        <input type="text" name="pin" maxlength="6" inputmode="numeric" required
                            class="w-full px-4 py-4 bg-slate-50 dark:bg-slate-800/50 border-2 border-slate-200 dark:border-white/5 rounded-xl text-2xl text-center font-mono tracking-[0.4em] text-emerald-600 outline-none"
                ...
                <div class="flex gap-2.5 pt-1">
                    <button type="button" @@click="reset()" 
                        class="flex-1 py-4 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black rounded-xl text-[10px] uppercase transition-all">BATAL</button>
